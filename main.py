...
movement_data_file = open('../Crowd-Analysis/processed_data/movement_data.csv', 'w') 
crowd_data_file = open('../Crowd-Analysis/processed_data/crowd_data.csv', 'w')
...
if IS_CAM:
    VID_FPS = VIDEO_CONFIG["CAM_APPROX_FPS"]
else:
    VID_FPS = cap.get(cv2.CAP_PROP_FPS)

# Ensure VID_FPS is non-zero before calling video_process function
if VID_FPS != 0:
    processing_FPS = video_process(cap, FRAME_SIZE, net, ln, encoder, tracker, movement_data_writer, crowd_data_writer, VID_FPS)
    # ... (rest of your code)
else:
    print("Error: Invalid FPS value. Make sure the video capture is working correctly.")
...
print("VID_FPS before calculation:", VID_FPS)
VID_FPS = cap.get(cv2.CAP_PROP_FPS)
print("VID_FPS after calculation:", VID_FPS)

if VID_FPS <= 0:
    print("Error: Invalid FPS value. Make sure the video capture is working correctly.")
    # Set a default FPS value or exit the program gracefully
    VID_FPS = 30
...
with open('../Crowd-Analysis/processed_data/video_data.json', 'w') as video_data_file:
